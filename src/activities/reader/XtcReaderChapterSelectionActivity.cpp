...
void XtcReaderChapterSelectionActivity::render(RenderLock&&) {
  ...
  for (int i = pageStartIndex; i < static_cast<int>(chapters.size()) && i < pageStartIndex + pageItems; i++) {
    ...
    const int titleFont = UiCjkFont::fontForText(renderer, title, UI_10_FONT_ID);
    const auto truncatedTitle = renderer.truncatedText(titleFont, title, contentWidth - 40);
    renderer.drawText(titleFont, contentX + 20, 60 + contentY + (i % pageItems) * 30, truncatedTitle.c_str(),
                      i != selectorIndex);
  }
